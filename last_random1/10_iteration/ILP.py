...
p_job = []
init_job_num = 20#initil
job_num = 0
rack_number = 4
oxc_per_rack = 64
ts_number = 100
ts_length = 1
inc_lim = [10 for i in range(0, rack_number)]
b_in_rack = [2560 for i in range(0, rack_number)]
# b_out_rack = [40 * 64 for i in range(0, rack_number)]
oxc_port = [64 for i in range(0, rack_number)]
worker_job = np.array(worker_job[0:init_job_num])
Data_job = np.array(Data_job[0:init_job_num])
w_t, _,p_job = job_deploy([64 for i in range(0, rack_number)], init_job_num, worker_job, rack_number)
print("pending:",p_job)
t_x, r = ilp(init_job_num, ts_number, ts_length, rack_number, PS_time, inc_lim, w_t, b_in_rack, Data_job, 40, oxc_port, 0.2, 1) 
print("t:",t_x,"r:",r)
# while len(p_job) !=0:
#     job_num = len(p_job)
#     worker_job = np.array(worker_job[init_job_num-job_num:init_job_num])
#     PS_time = np.array(PS_time[init_job_num-job_num:init_job_num])
#     Data_job = np.array(Data_job[init_job_num-job_num:init_job_num])
#     w_t, _,p_job = job_deploy([64 for i in range(0, rack_number)], job_num, worker_job, rack_number)  
#     t_x, r = ilp(job_num, ts_number, ts_length, rack_number, PS_time, inc_lim, w_t, b_in_rack, Data_job, 40, oxc_port, 0.2, 1)
#     print("t:",t_x,"r:",r)
